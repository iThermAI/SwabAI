...
def show_boxes(image, mouth_coords, face_mouth_coords):
    ...
    image = draw_point(image, face_mouth_coords[0], (255, 255, 0))
    image = draw_point(image, face_mouth_coords[1], (0, 255, 255))
...
def mask_bb(mask):
    cnt = remove_noise_return_contour(mask)

    # check if contour is suitable
...
def coords_avg(box_coords, line_coords):
...
def check_overlap(
    ...
    avg_x, avg_y = coords_avg(box_coords, line_coords)
